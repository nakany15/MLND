...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=False, epsilon=1.0, alpha=0.5):
        ...
        self.t = 0
        self.state_init = [['left', 'right', 'forward'],       #waypoint
                           ['red', 'green'],                   #light
                           ['left', 'right', 'forward', None], #left
                           ['left', 'right', 'forward', None], #right
                           ['left', 'right', 'forward', None]  #oncoming
                          ]
        self.q_temp = dict((action, 0.0) for action in self.valid_actions)
        for state_tuple in itertools.product(*self.state_init):
            self.Q[state_tuple] = self.q_temp.copy()
    ...
    def choose_action(self, state):
        ...
        from numpy.random import binomial, choice, random

        if self.learning == False or random() <= self.epsilon:
